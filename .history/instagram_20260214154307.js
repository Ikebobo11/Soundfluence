...
function payWithPaystack(btn, packageName, price) {
    ...
    const igLink = parentForm.querySelector('.link-input').value;
    ...
    const rule = parentForm.querySelector('.instuction-input') ? parentForm.querySelector('.instuction-input').value : "N/A";


    if (!customerEmail || !igLink) {
        alert("Please enter your email and Instagram link!");
        return;
    }

    // 3. Setup Paystack
    const handler = PaystackPop.setup({
        key: 'pk_live_618876bb15a280633047dd3afde67530eeddfc0b', 
        email: customerEmail,
        amount: price * 100, 
        currency: 'NGN',
        metadata: {
            custom_fields: [
                ...
                {
                    display_name: "Rule Link",
                    variable_name: "Rule_link",
                    value: rule
                },
            ]
        },
